...
from src.api.conversation import (
    ...
    _create_common_llm_params,
    _get_session_memory,
    _validate_and_get_tools,
    _get_error_fallback_message,
    _create_react_agent,
    _get_langfuse_handler_with_trace,
...
class TestHelperFunctions:
    ...
    def test_create_common_llm_params(self, sample_conversation_request):
        """Test creation of common LLM parameters"""
        request = ConversationRequest(**sample_conversation_request)
        params = _create_common_llm_params(request)
    ...
    def test_get_session_memory_new_session(self):
    ...
    def test_get_session_memory_existing_session(self, mock_session_memory):
        """Test getting memory for existing session"""
        session_id = "existing-session-123"
        
        # First call creates the memory
        memory1 = _get_session_memory(session_id)
    ...
    def test_validate_and_get_tools_success(self, mock_wrapper_config):
        """Test successful tool validation"""
        with patch('src.api.conversation.wrap_search_course_service') as mock_search:
            mock_tool = Mock()
            mock_tool.name = "search_course"
            mock_search.return_value = mock_tool
            
            tools = _validate_and_get_tools(["search_course"], "Bearer test-token")
            
            assert len(tools) > 0
            
    def test_validate_and_get_tools_invalid_tool(self):
        """Test tool validation with invalid tool name"""
        with pytest.raises(Exception) as exc_info:
            _validate_and_get_tools(["invalid_tool"], "Bearer test-token")
            
        assert "not found" in str(exc_info.value).lower()
        
    def test_get_error_fallback_message_connection(self):
        """Test error fallback message for connection error"""
        message = _get_error_fallback_message("Connection timeout")
        
        assert "connection" in message.lower()
        
    def test_get_error_fallback_message_timeout(self):
        """Test error fallback message for timeout error"""
        message = _get_error_fallback_message("Request timeout exceeded")
        
        assert "timeout" in message.lower() or "longer" in message.lower()
        
    def test_get_error_fallback_message_generic(self):
        """Test error fallback message for generic error"""
        message = _get_error_fallback_message("Unknown error")
        
        assert "technical difficulties" in message.lower() or "try again" in message.lower()


class TestPrepareMessages:
    """Test cases for _prepare_messages function"""
    
    def test_prepare_messages_empty_history(self, mock_session_memory):
        """Test preparing messages with empty history"""
        from src.api.conversation import _prepare_messages
        
        messages = _prepare_messages(mock_session_memory, "Hello")
        
        assert len(messages) == 1
        assert isinstance(messages[0], HumanMessage)
        assert messages[0].content == "Hello"
        
    def test_prepare_messages_with_history(self, mock_session_memory):
        ...
        messages = _prepare_messages(mock_session_memory, "New question")
...
class TestSaveConversationContext:
    """Test cases for _save_conversation_context function"""
    
    def test_save_conversation_context_success(self, mock_session_memory):
        """Test successful conversation context saving"""
        from src.api.conversation import _save_conversation_context
        
        _save_conversation_context(mock_session_memory, "User input", "AI output")
...
class TestConversationEndpoints:
    ...
    def test_clear_memory_success(self, client):
        ...
        from src.api.conversation import memory_by_session
...
class TestLLMInstanceCreation:
    ...
    def test_create_qwen_openai_instance_success(self, sample_conversation_request, mock_settings):
        ...
        from src.api.conversation import _create_qwen_openai_instance
        
        with patch('src.api.conversation.settings') as mock_settings_obj:
            ...
            request = ConversationRequest(**sample_conversation_request)
            llm = _create_qwen_openai_instance(request)
    ...
    def test_create_openai_instance_success(self, sample_conversation_request):
        ...
        from src.api.conversation import _create_openai_instance
        ...
        with patch('src.api.conversation.settings') as mock_settings_obj:
            ...
            request = ConversationRequest(**sample_conversation_request)
            llm = _create_openai_instance(request)
    ...
    def test_get_llm_instance_qwen_enabled(self, sample_conversation_request):
        ...
        from src.api.conversation import _get_llm_instance
...
class TestStreamingFunctions:
    ...
    async def test_create_agent_response_stream_success(self, sample_conversation_request, mock_session_memory):
        ...
        from src.api.conversation import _create_agent_response_stream
        ...
        chunks = []
        async for chunk in _create_agent_response_stream(
    ...
    async def test_create_agent_response_stream_error(self, sample_conversation_request, mock_session_memory):
        ...
        chunks = []
        async for chunk in _create_agent_response_stream(
            mock_graph, messages, mock_session_memory, "Hi"
        ):
            chunks.append(chunk)
...
class TestAgentCreation:
    ...
    def test_create_react_agent_with_tools(self, sample_conversation_request):
        """Test creating react agent with tools"""
        from src.api.conversation import _create_react_agent
        
        with patch('src.api.conversation.create_react_agent') as mock_create_agent:
            with patch('src.api.conversation.get_agent_system_prompt_for_workflow') as mock_prompt:
                with patch('src.api.conversation._get_langfuse_handler_with_trace') as mock_langfuse:
                    ...
                    result = _create_react_agent(mock_llm, mock_tools, "general")
                    
                    assert result == mock_graph
                    mock_create_agent.assert_called_once()
                    
    def test_create_react_agent_without_tools(self, sample_conversation_request):
        ...
        with patch('src.api.conversation.create_react_agent') as mock_create_agent:
            with patch('src.api.conversation.get_agent_system_prompt_for_workflow') as mock_prompt:
                with patch('src.api.conversation._get_langfuse_handler_with_trace') as mock_langfuse:
                    ...
                    result = _create_react_agent(mock_llm, [], "general")
...
class TestLangfuseIntegration:
    ...
    def test_get_langfuse_handler_with_trace_enabled(self):
        """Test Langfuse handler creation when enabled"""
        from src.api.conversation import _get_langfuse_handler_with_trace
        
        with patch('src.api.conversation.settings') as mock_settings:
            with patch('src.api.conversation.langfuse') as mock_langfuse:
                ...
                result = _get_langfuse_handler_with_trace("session-123", "general", "Hello")
                
                # Should return handler
                assert result is not None or result is None  # May vary based on implementation
                
    def test_get_langfuse_handler_with_trace_disabled(self):
        ...
        for workflow in workflows:
            with patch('src.api.conversation.create_react_agent') as mock_create_agent:
                with patch('src.api.conversation.get_agent_system_prompt_for_workflow') as mock_prompt:
                    ...
                    _create_react_agent(mock_llm, [], workflow)
...
class TestMessagePreparation:
    ...
    def test_save_conversation_context(self, mock_session_memory):
        """Test saving conversation context"""
        from src.api.conversation import _save_conversation_context
        
        _save_conversation_context(mock_session_memory, "User input", "AI response")
...
class TestErrorHandling:
    ...
    def test_get_error_fallback_message_various_errors(self):
        ...
        for error_msg in error_cases:
            result = _get_error_fallback_message(error_msg)

...
class TestToolIntegration:
    ...
    def test_tool_selection_for_tutor_search(self):
        ...
        with patch('src.core.utils.api_utils.wrap_search_tutor_service') as mock_wrap:
            mock_tool = Mock()
            mock_tool.name = "search_tutor"
            mock_wrap.return_value = mock_tool
            
            tools = validate_and_get_tools(["search_tutor"], None)
    ...
    def test_multiple_tool_selection(self):
        ...
        with patch('src.core.function_tools.wrap_search_course_service') as mock_course:
            with patch('src.core.function_tools.wrap_search_tutor_service') as mock_tutor:
...
class TestChatEndpoint:
    ...
    def test_chat_endpoint_basic_request(self):
        ...
        with patch('src.core.utils.llm_factory.get_llm_instance') as mock_llm:
            with patch('src.core.utils.memory_manager.get_session_memory') as mock_mem:
                with patch('src.core.utils.memory_manager.prepare_messages') as mock_prep:
                    with patch('src.core.utils.memory_manager.save_conversation_context') as mock_save:
                        mock_model = Mock()
                        mock_response = Mock()
...
class TestSessionManagement:
    ...
    def test_clear_memory_existing_session(self):
        ...
        from src.core.utils.memory_manager import get_session_memory, memory_by_session
...
class TestWorkflowSelection:
    ...
    def test_assistant_workflow_uses_standard_model(self):
        ...
        with patch('src.core.utils.llm_factory.settings') as mock_settings:
            ...
            assert result.model_name == "qwen-base"
